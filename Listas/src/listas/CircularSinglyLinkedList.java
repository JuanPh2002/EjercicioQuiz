...
public class CircularSinglyLinkedList<T extends Comparable> implements Ilist<T> {
    ...
    public void deleteAbove(T d) throws Exception {
        ...
        while (current != head) {
            if (current.getData().compareTo(head.getData()) == 0 && head.getData().compareTo(d) == 1) {
                delete();
            } else if ( current.getData().compareTo(tail.getData()) == 0 && tail.getData().compareTo(d) == 1) {
                deleteLast();
            } else if (current.getData().compareTo(d) == 1) {
                deleteSpecific(current.getData());
            }
